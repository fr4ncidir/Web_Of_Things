...
def switch_handler(added_bindings,result_file,ignore=[]):
    if not added_bindings:
        ...
        full_added = {}
        full_added["head"] = {}
        full_added["head"]["vars"] = [ key for item in added_bindings for key in item.keys() ]
        full_added["results"] = {}
...
class TestCase2_QueryUpdate(unittest.TestCase):
    ...
    def test_6(self):
        ...
        for index,action in enumerate(actions):
            ...
            def confirm_handler(added,removed):
                self.assertTrue(switch_handler(added,
                    "res_new_confirmation_ts.json",ignore=["ts"]))
                if added:
                    self.engine.unsubscribe(subids["confirm"])
            def complete_handler(added,removed):
                self.assertTrue(switch_handler(added,
                    "res_new_completion_ts.json",ignore=["ts"]))
                if added:
            ...
            instance,subids = action.newRequest(bindings,
                confirm_handler=confirm_handler, completion_handler=complete_handler) 
            ...
            def out_handler(added,removed):
                self.assertTrue(switch_handler(added,"res_new_instance_output.json"))
                if added:
                    self.engine.unsubscribe(subids["output"])
            instance,subids = action.newRequest(bindings,
                confirm_handler=confirm_handler,
                completion_handler=complete_handler,output_handler=out_handler) 
            ...
            if action.type == AType.INPUT_ACTION:
                ...
                deep_actions[index].post_output({"instance": instance,
            ...
            deep_actions[index].disable()
            deep_actions[index].deleteInstance(instance)
        self.test_0()
    ...
    def test_7(self):
        ...
        deep_events = [  Event.buildFromQuery(self.engine,"<http://MyFirstWebThing.com/Event1>"),
                    Event.buildFromQuery(self.engine,"<http://MyThirdWebThing.com/Event1>")]
        
        # Adding the instances
        for index,event in enumerate(deep_events):
            ...
            def event_monitor(added,removed):
                ...
                if added:
                    if notification_iteration == 0:
                        self.assertTrue(switch_handler(added,"res_new_{}_event_instance.json".format(event.type.value.lower()),ignore=["eTS"]))
                    else:
                        self.assertTrue(switch_handler(added,"res_new_{}_event_instance_update.json".format(event.type.value.lower()),ignore=["eTS"]))
